...
const userTextInput = document.getElementById('user-input');
...
async function fetchAgentResponse(userText) {
  const WEBHOOK_URL = 'https://primary-production-8cc1.up.railway.app/webhook/lucia';
  ...
  if (!res.ok) throw new Error(`Agente respondeu status ${res.status}`);
...
streamWordButton.onclick = async () => {
  try {
    ...
    if (!userText) return alert('Digite algo primeiro.');

    userTextInput.value = '';
    userTextInput.focus();

    const agentText = await fetchAgentResponse(userText);
    console.log('Texto do agente:', agentText);

    let chunks = agentText.split(' ');
    chunks.push(''); // chunk vazio indica fim
    ...
    console.error('Erro ao streamar texto:', err);
  } finally {
    streamWordButton.disabled = false;
  }
};
...
streamAudioButton.onclick = async () => {
  try {
    ...
    if (!elevenKey) return alert('Coloque a elevenlabsKey em api.json');
    ...
    /* ------- converte texto → Pulse-Code Modulation ------- */
    const pcmStream = await elevenLabsStream(agentText, elevenKey);

    let i = 0;
    for await (const chunk of pcmStream) {
      for (const part of splitArrayIntoChunks([...chunk], 10000)) {
        sendStreamMessage([...part], i++);
      }
    }
    sendStreamMessage([], i); // chunk vazio encerra
    console.log('Áudio finalizado');
  } catch (err) {
    console.error('Erro ao streamar áudio:', err);
  } finally {
    streamAudioButton.disabled = false;
  }
};

// voiceId = '21m00Tcm4TlvDq8ikWAM' mulher
// voiceId = '29vD33N1CtxCmqQRPOHJ' homem
async function elevenLabsStream(text, key, voiceId = '29vD33N1CtxCmqQRPOHJ') {
  const r = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}/stream?output_format=pcm_16000`, {
    method: 'POST',
